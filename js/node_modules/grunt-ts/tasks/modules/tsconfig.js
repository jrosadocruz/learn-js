...
export function resolveAsync(applyTo, taskOptions, targetOptions, theTemplateProcessor, theGlobExpander = null, theVerboseLogger = null) {
    templateProcessor = theTemplateProcessor;
    globExpander = theGlobExpander;
    gruntfileGlobs = getGlobs(taskOptions, targetOptions);
    verboseLogger = theVerboseLogger || ((logText) => { });
    return new Promise((resolve, reject) => {
        ...
        if (applyTo.tsconfig.passThrough) {
            ...
            let projectSpec = { extends: applyTo.tsconfig.tsconfig };
...
function handleBadConfiguration(options, projectSpec) {
    ...
    const tsconfigSetting = options.tsconfig;
...
function applyCompilerOptions(applyTo, projectSpec) {
    let result = applyTo || {};
    const co = projectSpec.compilerOptions, tsconfig = applyTo.tsconfig;
...
function addFilesToCompilationContext(applyTo, projectSpec) {
    const resolvedInclude = [], resolvedExclude = [], resolvedFiles = [];
    const result = applyTo, co = projectSpec.compilerOptions, tsconfig = applyTo.tsconfig, src = applyTo.CompilationTasks[0].src;
    ...
    const expandedCompilationContext = [];
...
export function prettyJSON(object, indent = 4, newLine = utils.eol) {
    const cache = [];
    let value = JSON.stringify(object, function (key, value) {
