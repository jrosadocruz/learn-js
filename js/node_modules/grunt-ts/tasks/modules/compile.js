...
export var grunt = require('grunt');
var executeNode;
...
export function compileResultMeansFastCacheShouldBeRefreshed(options, result) {
    return (options.fast !== 'never' &&
        (result.code === 0 || (result.code === 2 && !options.failOnTypeErrors)));
}
export function compileAllFiles(options, compilationInfo) {
    ...
    if (compilationInfo.outDir && options.baseDir && files.length > 0 && !options.rootDir) {
        const baseDirFile = '.baseDir.ts', baseDirFilePath = path.join(options.baseDir, baseDirFile), settingsSource = !!tsconfig ? 'tsconfig.json' : 'Gruntfile ts `options`', settingsSection = !!tsconfig ? 'in the `compilerOptions` section' : 'under the task or ' +
    ...
    if (tsconfig && tsconfig.passThrough) {
        ...
        if (options.module) {
            const moduleOptionString = ('' + options.module).toLowerCase();
